...
Builds also write per-set checkpoint files under `.tcgjson-cache/set-checkpoints` by default. Checkpoints are separated by product-line slug and set ID. They are useful for resuming interrupted local runs, but they duplicate normalized product rows and can be regenerated from TCGplayer set data plus any local product-detail cache. For that reason, set checkpoints are ignored by git unless you explicitly place them elsewhere with `--checkpoint-dir`. Use `--no-checkpoints` to disable them entirely.
