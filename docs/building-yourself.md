...
Builds also write per-set checkpoint files under `.tcgjson-cache/set-checkpoints` by default. Checkpoints are separated by product-line slug and source; refreshed product rows are sourced from search so normalized objects stay consistent between releases. They are useful for resuming interrupted local runs, but they duplicate normalized product rows and can be regenerated from TCGplayer set data plus any local product-detail cache. For that reason, set checkpoints are ignored by git unless you explicitly place them elsewhere with `--checkpoint-dir`. Use `--no-checkpoints` to disable them entirely.
