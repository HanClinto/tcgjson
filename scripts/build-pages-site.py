...
def markdown_to_html(markdown: str, source_dir: Path) -> str:
    ...
    while index < len(lines):
        ...
        if heading:
            ...
            heading_id = slugify(plain)
            anchor = f'<a class="heading-anchor" href="#{heading_id}" aria-label="Link to {html.escape(plain, quote=True)}">#</a>'
            output.append(f'<h{level} id="{heading_id}">{anchor}{text}</h{level}>')
            index += 1
            continue
